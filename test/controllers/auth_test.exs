defmodule Rumbl.AuthTest do
  ...
  test "call places user from from session into assigns", %{conn: conn} do
    user = insert_user()
    conn = 
      conn
  ...
  test "login with a valid name/password", %{conn: conn} do
  ...
  test "login without a foud user", %{conn: conn} do
  ...
  test "with password mismatch", %{conn: conn} do
    ...
    assert {:error, :unauthorized, _conn} = 
      Auth.login_by_username_and_pass(conn, "me", "notsecret", repo: Repo)
  end
end
